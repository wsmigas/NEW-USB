...
export const ImportCsvModal: React.FC<ImportCsvModalProps> = ({
  ...
  const [isSyncingServerReports, setIsSyncingServerReports] = useState<boolean>(false);
  const [syncServerResult, setSyncServerResult] = useState<{
  ...
  const handleSyncServerReports = async () => {
    setIsSyncingServerReports(true);
    setReportsError('');
    setSyncServerResult(null);
    try {
      const res = await StorageService.syncServerReportsAsync();
      if (!res.success) {
        setReportsError(res.error || 'Erro ao sincronizar pasta de relatórios.');
        return;
      }
      setSyncServerResult({
        totalFilesFound: res.totalFilesFound,
        matchedCount: res.matchedCount,
        unmatchedCount: res.unmatchedCount
      });
      if (res.matchedCount > 0) {
        onImportComplete(res.matchedCount, 0, [
          `Sincronização concluída: ${res.matchedCount} relatórios associados a discos da pasta reports/.`
        ]);
        if (onReportsImported) {
          onReportsImported(res.matchedCount, res.totalFilesFound);
        }
      }
    } catch (err: any) {
      setReportsError(`Erro ao comunicar com servidor: ${err?.message || 'Erro desconhecido'}`);
    } finally {
      setIsSyncingServerReports(false);
    }
  };
        ...
        <div className="p-6">
          ...
          {activeTab === 'reports' && (
            <div className="space-y-4">
              ...
              {/* Server Folder Quick Sync Card */}
              <div className="p-3.5 rounded-xl border bg-slate-50 dark:bg-[#12151e] border-slate-200 dark:border-slate-800">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 block">
                      Ficheiros copiados para a pasta <code className="text-cyan-600 dark:text-cyan-400 font-mono">reports/</code> do servidor?
                    </span>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400">
                      O servidor analisa os ficheiros <code className="font-mono">.html</code> no disco e faz a ligação automática a cada disco pelo Ticket, ID ou Número de Série.
                    </span>
                  </div>

                  <button
                    type="button"
                    onClick={handleSyncServerReports}
                    disabled={isSyncingServerReports}
                    className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold text-white bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 shrink-0 shadow-sm transition-all"
                  >
                    {isSyncingServerReports ? (
                      <>
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                        <span>A varrer pasta reports/...</span>
                      </>
                    ) : (
                      <>
                        <RefreshCw className="w-3.5 h-3.5" />
                        <span>Sincronizar Pasta reports/</span>
                      </>
                    )}
                  </button>
                </div>
                ...
                {syncServerResult && (
                  <div className="mt-2.5 pt-2.5 border-t border-slate-200 dark:border-slate-800 flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400">
                    <CheckCircle2 className="w-4 h-4 shrink-0" />
                    <span>
                      Encontrados <strong>{syncServerResult.totalFilesFound}</strong> ficheiros. <strong>{syncServerResult.matchedCount}</strong> relatórios foram associados com sucesso aos discos!
                      {syncServerResult.unmatchedCount > 0 && ` (${syncServerResult.unmatchedCount} sem correspondência direta)`}
                    </span>
                  </div>
                )}
              </div>
